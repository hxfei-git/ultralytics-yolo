...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster','C3k2_CGFF_10','C3k2_CGFF_25','C3k2_CGFF_50','C3k2_CGFF_75','C3k2_CGFF_90', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'RCM', 'PyramidContextExtraction', 'DynamicInterpolationFusion', 'FuseBlockMulti', 'FeaturePyramidSharedConv', 'C3k2_FMB', 'LDConv', 'C3k2_gConv', 'C3k2_WDBB', 'C3k2_DeepDBB', 'EnhanceLocalSPPF', 'RES_EnhanceLocalSPPF',"CAF_EnhanceLocalSPPF", "CAF_RES_EnhanceLocalSPPF", "ASCSPP", "ACSPP", "ACSPP_1", "DSSPP",
           ...
           'C3k2_CSSC', 'C3k2_CNCM', 'C3k2_HFRB', 'C3k2_EVA', 'C3k2_RMBC', 'C3k2_RMBC_LA', 'C3k2_IEL', 'C3k2_SFMB', 'CGF_1', 'CGF_2', 'CGF_3', 'FAFF', 'LFAFF', 'FSCF', 'CGFF_25'
           ]
...
class ASCSPP(nn.Module):  # Adaptive Shared Convolutional SPP
    def __init__(self, c1, c2, k=5):
        super().__init__()
        c_ = c1 // 2
        self.cv1 = Conv(c1, c_, 1, 1)
        
        # 共享的局部 Block（depth-wise + point-wise）
        self.block = nn.Sequential(
            Conv(c_, c_, k, 1, g=c_),  # depth-wise conv
            Conv(c_, c_, 1, 1)         # point-wise conv
        )
        
        # 全局上下文分支
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        self.global_conv = Conv(c_, c_, 1, 1)  # 1x1 conv after pooling
        
        # 融合所有分支：x1, x2, x3, x4 + global
    ...
    def forward(self, x):
        ...
        # 全局分支：GAP → 1x1 Conv → Upsample to match spatial size
        global_feat = self.global_pool(x1)              # [B, c_, 1, 1]
        global_feat = self.global_conv(global_feat)     # [B, c_, 1, 1]
        global_feat = F.interpolate(global_feat, size=x1.shape[2:], mode='bilinear', align_corners=False) # [B, c_, H, W]

        out = torch.cat([x1, x2, x3, x4, global_feat], dim=1)
        return self.cv2(out)
    
class ACSPP(nn.Module):  # Adaptive Shared Convolutional SPP
    def __init__(self, c1, c2, k=3):
        super().__init__()
        c_ = c1 // 2
        self.cv1 = Conv(c1, c_, 1, 1)
        
        # Block1（depth-wise + point-wise）
        self.block1 = nn.Sequential(
            Conv(c_, c_, k, 1, g=c_),  # depth-wise conv
            Conv(c_, c_, 1, 1)         # point-wise conv
        )
...
class CAF_EnhanceLocalSPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        y_enhanced = self.caf(x_align, y)

        return y_enhanced
    
class CAF_RES_EnhanceLocalSPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        return y_enhanced + y if self.add else y_enhanced
...
######################################## Cross-Guided Fusion start ########################################

class CGF_1(nn.Module):
    """
    Cross-Guided Fusion (CGF_1) Module with Channel Alignment
    ...
    """
    def __init__(self, inc, ouc, res=False):
        super(CGF_1, self).__init__()
        ...
        # Step 1: 统一通道数
        self.align1 = Conv(c1, ouc, 1)
        self.align2 = Conv(c2, ouc, 1)
        ...
        # Step 3: 注意力模块
        self.spatial1 = Spatial(self.sp_part)  # x1 → att for x2
        self.spatial2 = Spatial(self.sp_part)  # x2 → att for x1
        self.channel1 = Channel(self.ch_part)
        self.channel2 = Channel(self.ch_part)
    ...
    def forward(self, inputs):
        ...
        x2_sp_mod = x2_sp * self.spatial1(x1_sp)          # (B, sp, H, W)
        x2_ch_mod = x2_ch * self.channel1(x1_ch)          # (B, ch, H, W)
        x2_fused = torch.cat([x2_sp_mod, x2_ch_mod], dim=1)

        # Step 4: 用 x2 的注意力调制 x1
        x1_sp_mod = x1_sp * self.spatial2(x2_sp)          # (B, sp, H, W)
        x1_ch_mod = x1_ch * self.channel2(x2_ch)          # (B, ch, H, W)
        x1_fused = torch.cat([x1_sp_mod, x1_ch_mod], dim=1)

        # Step 5: 拼接所有部分
        if self.res:
            out = torch.cat([x1_aligned + x1_fused, x2_aligned + x2_fused], dim=1)  # (B, 2*ouc, H, W)
        else :
...
class CGF_2(nn.Module):
    """
    Cross-Guided Fusion (CGF_2) Module with Channel Alignment
    - First aligns input channels via 1x1 conv
    - Then splits each feature into attention part and content part
    - Uses mutual guidance: one generates attention to modulate the other
    - Finally concatenates all parts and projects to target output dimension
...
class CGF_3(nn.Module):
    """
    Cross-Guided Fusion (CGF_3) Module with Channel Alignment
    - First aligns input channels via 1x1 conv
    - Then splits each feature into attention part and content part
    - Uses mutual guidance: one generates attention to modulate the other
    - Finally concatenates all parts and projects to target output dimension
